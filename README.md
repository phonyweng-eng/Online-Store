...
<div class="cart">
    ...
    <h3>Checkout</h3>
    <!-- The names here are what will show up in your email -->
...
async function placeOrder(){
    ...
    // Build the string for the email
    let orderList = cart.map(item => `${item.name} ($${item.price})`).join(", ");

    // Send to Formspree (REPLACE 'your_id' WITH YOUR ACTUAL FORMSPREE ID)
    try {
        ...
        if (response.ok) {
            alert("Order Confirmed! Email sent to shop.");
            cart = [];
            updateCart();
            ...
            alert("Delivery failed. Please check your internet.");
        }
    } catch (err) {
